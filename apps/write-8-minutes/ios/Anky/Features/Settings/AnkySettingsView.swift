...
struct AnkySettingsView: View {
    ...
    private var dailyTargetFootnote: String {
        if let pendingTargetMinutes, pendingTargetMinutes != effectiveTargetMinutes {
            return "Today stays at \(effectiveTargetMinutes) min · \(pendingTargetMinutes) min from tomorrow. Writing this long opens your apps for the rest of the day."
        }
        return "Writing this long opens your apps for the rest of the day. Sessions are never cut short — you can always keep going."
    }
    ...
    private var iCloudSubtitle: String {
        if viewModel.isICloudBackupEnabled {
            if let date = viewModel.iCloudBackupLastDate {
                return "Encrypted · last backup \(date.formatted(date: .abbreviated, time: .shortened))"
            }
                ...
                HStack(spacing: 14) {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        Text(AnkyLocalization.ui("Anky"))
                            .font(.ankyLabel)
                            .foregroundStyle(Color.ankyInk)
                        Text(AnkyLocalization.ui("Version \(viewModel.appVersion)"))
                            .font(.ankyCaption)
                            .foregroundStyle(Color.ankyInkSoft)
                    }
                    Spacer()
