...
struct MapView: View {
        ...
        NavigationStack(path: $path) {
            ...
            .navigationDestination(for: MapRoute.self) { route in
                ...
                case .reveal(let artifact):
                    RevealView(
                        viewModel: RevealViewModel(artifact: artifact),
                        writeBeforeScrollUnlockGrant: writeBeforeScrollUnlockGrant,
                        onDeleted: viewModel.refresh,
                        onTryAgain: {
                            continueWriting(from: artifact)
                        },
                        ...
                        onWriteBeforeScrollUnlock: onWriteBeforeScrollUnlock
                    )
                }
            }
            .onAppear {
    ...
    private func presentReflectionReadyBubble(for artifact: SavedAnky) {
        ankyCompanion.witness(
            ...
            sequence: .waveFront,
            bubble: AnkyBubble(
                text: AnkyLocalization.ui("your reflection is ready."),
                actions: [
                    AnkyChatAction(AnkyLocalization.ui("open reflection"), isPrimary: true) {
                        path = [.reveal(artifact)]
    ...
    var body: some View {
        GeometryReader { geometry in
            Canvas { context, size in
                ...
                context.stroke(
                    path,
                    with: .color(Color.ankyPaper.opacity(0.13)),
                    style: StrokeStyle(lineWidth: 10, lineCap: .round, lineJoin: .round)
                )
                context.stroke(
                    path,
                    with: .color(Color.ankyPaper.opacity(0.28)),
...
private struct TrailDayNode: View {
    ...
    private var accessibilityLabel: String {
        let date = day.isToday ? AnkyLocalization.ui("Today") : formattedUTCDate(day.date, dateFormat: nil)
        return "\(date), \(AnkyLocalization.ui(day.trailActivitySummary))"
    }
        ...
        if day.isToday {
            if let image = UIImage(named: "today-anky-icon") {
                ZStack {
                    ...
                    Circle()
                        .fill(Color.ankyInk.opacity(0.42))
                        .frame(width: 68, height: 68)
    ...
    private func dayCircle(size: CGFloat, fontSize: CGFloat) -> some View {
        ZStack {
            Circle()
                .fill(Color.ankyInk.opacity(day.hasAnky ? 0.76 : 0.58))
            ...
            CircleTextureLines()
                .stroke(Color.ankyPaper.opacity(day.hasAnky ? 0.08 : 0.05), lineWidth: 1)
                .frame(width: size, height: size)
                .clipShape(Circle())
            ...
            Circle()
                .fill(
                    RadialGradient(
                        colors: [
                            Color.ankyPaper.opacity(day.hasAnky ? 0.10 : 0.05),
                            Color.clear
                        ],
                        center: .topLeading,
                        startRadius: 1,
    ...
    private var nodeTexture: LinearGradient {
        LinearGradient(
            colors: [
                Color.ankyPaper.opacity(0.10),
                Color.clear,
                Color.ankyInk.opacity(day.hasAnky ? 0.22 : 0.34)
            ],
    ...
    private var nodeStroke: Color {
        ...
        return day.hasAnky ? nodeFill.opacity(0.76) : Color.ankyPaper.opacity(0.18)
    }
    ...
    private var nodeSymbolColor: Color {
        day.hasAnky ? Color.ankyPaper.opacity(0.92) : Color.ankyPaper.opacity(0.52)
    }
}
...
private struct DayCompletionMarker: View {
    var body: some View {
        Circle()
            ...
            .overlay(
                Circle()
                    .stroke(Color.ankyInk.opacity(0.62), lineWidth: 3)
            )
...
private struct MapStatsPanel: View {
    ...
    var body: some View {
        Button(action: action) {
            ...
            .shadow(color: Color.ankyViolet.opacity(0.16), radius: 14, y: 6)
        }
        .buttonStyle(.plain)
...
private struct MissingAnkyView: View {
    var body: some View {
        VStack(spacing: 12) {
            Image(systemName: "doc.badge.questionmark")
                .font(.system(size: 34, weight: .semibold))
                .foregroundStyle(MapDayPalette.gold.opacity(0.78))
        ...
        .background(LazureWall(mood: .dawn))
    }
}

struct MapDayBackground: View {
    var body: some View {
        // Lazure: the day's reading room is the breathing paper wall.
        LazureWall(mood: .dawn)
    }
}

enum MapDayPalette {
    // Lazure remap: surfaces became paper, text became ink.
