...
final class UnlockLadderTests: XCTestCase {
    ...
    // MARK: Fix 3 — the daily grant survives an active Quick Pass

    func testDailyGrantIsOfferedWhenNoPassWasUsed() {
        ...
        XCTAssertEqual(action, .offer(grant))
    }
    ...
    func testDailyGrantIsOfferedWhenQuickPassExpiredMidSession() {
        let grant = dailyGrant(at: now)
        let action = ladder.action(
            grant: grant,
...
extension UnlockLadderTests {
    ...
    func testSubscriberAtTargetIsUnchangedByTheMomentBranch() {
        ...
        XCTAssertEqual(offered, .offer(grant))
    }
