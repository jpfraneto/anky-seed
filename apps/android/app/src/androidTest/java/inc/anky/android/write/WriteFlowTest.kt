...
class WriteFlowTest {
    ...
    @Test
    fun consecutiveIdenticalGlyphsRemainInWritingText() {
        compose.onNodeWithTag("write-input").performTextInput("l")
        compose.onNodeWithTag("write-input").performTextInput("l")
        compose.onNodeWithTag("write-visible-text").assertTextContains("ll", substring = true)
    }
