...
function PreviewWritingSurface({ onStart }: PreviewWritingSurfaceProps) {
  return (
    <button
      ...
      className="hidden md:relative w-full overflow-hidden rounded-lg border border-gold-200/18 bg-black/40 p-0 text-left shadow-[0_35px_120px_rgba(0,0,0,0.42)] transition hover:border-gold-200/35 focus:outline-none focus:ring-2 focus:ring-gold-300/70"
      type="button"
      onClick={() => onStart()}
    >
      <div
      ...
      <div className="relative min-h-[430px] px-6 py-6 sm:min-h-[520px] sm:px-8">
        <div className="flex items-center justify-between text-xs uppercase text-cream/54">
          ...
          <span>Start writing to enter Anky mode</span>
        </div>

        <div className="absolute bottom-8 right-7 max-w-[76%] text-right font-mono text-lg leading-8 text-cream/28 sm:text-xl">
          i am carrying more than i said out loud and maybe the page can hold it
          first
        </div>
        ...
        <div className="absolute left-1/2 top-[38%] grid h-[190px] w-[190px] -translate-x-1/2 -translate-y-1/2 place-items-center">
          ...
          <span className="relative font-serif text-5xl text-gold-100/90">
            a
          </span>
        </div>
      </div>
