...
- **8 minutes** is the full practice. **1-8 seconds** of stillness can close the writing surface before reflection; the default is 8 seconds. The terminal stillness marker is the configured millisecond value (`8000` by default, `1000` at the shortest), not a character and not writing duration.
- **No delete, no stop.** The keyboard has no backspace during a session. The timer does not pause. What is written is written.
- **The string IS the session.** Line 1: epoch-ms timestamp, space, first character. Every subsequent writing line: plain integer delta-ms, space, character. Typed spaces are encoded as `SPACE`; newlines are not accepted. SHA-256 of the exact UTF-8 string is the artifact identity. Perfectly deterministic, perfectly reconstructable. No screen may ever misrepresent this.
