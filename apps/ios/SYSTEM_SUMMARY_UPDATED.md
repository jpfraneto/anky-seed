...
No files were modified during the assessment pass. This report reflects the current repo at `/Users/kithkui/anky/apps/ios`.
...
| You/profile | `Anky/Features/You/YouViewModel.swift` | Account, export, backup, recovery, subscription, stats. |
...
Monetization is now RevenueCat-backed subscription access, not virtual-currency credit packs.

### RevenueCat

The purchase source of truth lives in `Anky/Purchases/AnkyPurchasesConfig.swift`.

It configures:

- Public Apple SDK key: `revenueCatPublicKey`, expected to be an `appl_...` key.
- Entitlement: `pro`
- Offering: `default`
...
RevenueCat receives purchase/customer identity data for subscription management.
...
| Reflection after writing | Implemented | `MirrorClient`, sealing flow | Remote/subscription UX needs production QA | Medium |
| Subscription paywall | Implemented | `Anky/Purchases`, settings/paywall surfaces | Dashboard/offering/product alignment must stay exact | High |
...
- Archive/You views
- `RevealViewModel`
- RevenueCat subscription/paywall layer
- New widget target
...
A live private writing/reflection app that has recently gained a serious WBS Screen Time spike. The original app is still present: onboarding, check-in dashboard, map, archive, You/profile, RevenueCat subscription gating, exports, recovery, and iCloud backup. The current launch path now strongly favors the writing interface.
