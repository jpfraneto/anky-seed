...
struct LocalAnkyArchive {
    ...
    static func reopenableDraftText(from sealedText: String) -> String {
        ...
        if let last = lines.last?.trimmingCharacters(in: .whitespacesAndNewlines),
           AnkyDuration.terminalMarkerMs(from: last) != nil {
            lines.removeLast()
        }
    ...
    private static func extractedProtocolBlock(from text: String) -> String? {
        ...
        for line in lines {
            ...
            if let normalized {
                ...
                if AnkyDuration.terminalMarkerMs(from: normalized) != nil {
    ...
    private static func normalizedProtocolLine(_ line: String) -> String? {
        ...
        if trimmedLine == line,
           let terminalMs = AnkyDuration.terminalMarkerMs(from: trimmedLine) {
            return "\(terminalMs)"
        }
