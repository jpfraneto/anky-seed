...
struct LevelSyncClient {
    ...
    /// Server-first account deletion. The body is intentionally empty and
    /// signed; the backend scopes deletion to the authenticated account.
    func deleteAccount(identity: WriterIdentity) async throws {
        var request = try signedRequest(path: "account", body: Data(), identity: identity)
