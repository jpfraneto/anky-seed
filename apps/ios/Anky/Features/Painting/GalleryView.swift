...
struct GalleryView: View {
    ...
    @State private var isInstallingStaticDefaults = false
    ...
    var body: some View {
        ...
        .task(id: currentLevel) {
            await installEarnedStaticDefaultsIfNeeded()
        }
    ...
    private func installEarnedStaticDefaultsIfNeeded() async {
        guard !isInstallingStaticDefaults else { return }
        let highestEarnedStaticLevel = min(currentLevel, Self.staticLevelMax)
        guard highestEarnedStaticLevel >= 2 else { return }

        let store = PaintingAssetStore()
        let missing = (2...highestEarnedStaticLevel).filter {
            store.installedPackage(forLevel: $0) == nil
        }
        guard !missing.isEmpty else { return }

        isInstallingStaticDefaults = true
        defer { isInstallingStaticDefaults = false }
        for level in missing {
            _ = try? await store.downloadPackage(level: level)
        }
        load()
    }
}
...
private struct LockedPaintingTile: View {
    ...
    var body: some View {
        VStack(spacing: 8) {
            ...
            Text(AnkyLocalization.ui("level short format", level))
                .font(.ankyCaption)
...
private struct GalleryDetailView: View {
    ...
    var body: some View {
            ...
            VStack(spacing: 22) {
                ...
                VStack(spacing: 8) {
                    ...
                    if entry.progress >= 1, entry.package.thresholdSeconds > 0 {
                        ...
                        Text(AnkyLocalization.ui("revealed by seconds format", entry.package.thresholdSeconds.formatted()))
                            .font(.ankyCaption)
                            .foregroundStyle(Color.ankyInkSoft)
                    }
                }
        ...
        .navigationTitle(AnkyLocalization.ui("level short format", entry.package.level))
