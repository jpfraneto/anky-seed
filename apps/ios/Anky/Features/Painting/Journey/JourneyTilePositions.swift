...
final class JourneyState: ObservableObject {
    ...
    func refresh(
        ...
        let sessionDays = Set(completeSessions.map { calendar.startOfDay(for: $0.createdAt) })

        var writtenIndices = Set<Int>()
        var missedIndices = Set<Int>()
        if let firstDay = JourneyAnchor.firstSealedDay(in: sessions, calendar: calendar) {
        ...
        minutesWritten = Int(completeSessions.reduce(Int64(0)) { $0 + $1.durationMs } / 60_000)
        writingsCount = completeSessions.count
