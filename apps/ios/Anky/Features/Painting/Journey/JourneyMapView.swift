...
struct JourneyCardView: View {
    ...
    var body: some View {
        ...
        .accessibilityLabel(AnkyLocalization.ui("Writing journey accessibility format", currentJourneyDay, JourneyGrid.totalTiles, completedTileCount))
    ...
    private var mapChrome: some View {
        VStack(spacing: 0) {
            HStack {
                Text(AnkyLocalization.ui("day count format", currentJourneyDay, JourneyGrid.totalTiles))
            ...
            Spacer()
            HStack {
                statPill(icon: .sun, text: AnkyLocalization.ui("mins written format", minutesWritten))
                Spacer()
                statPill(icon: .feather, text: AnkyLocalization.ui("writings count format", writingsCount))
            }
        }
        .padding(10)
        .allowsHitTesting(false)
