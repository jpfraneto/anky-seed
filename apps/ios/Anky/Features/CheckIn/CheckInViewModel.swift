...
final class CheckInViewModel: ObservableObject {
    ...
    var title: String {
        switch mode {
        case .write, .image:
            return AnkyLocalization.ui("Write what's here.")
        case .talk:
            return AnkyLocalization.ui("Talk what is alive.")
        }
    }
    ...
    var subtitle: String {
        switch mode {
        case .write, .image:
            return AnkyLocalization.ui("No need to explain it well.")
        case .talk:
            return AnkyLocalization.ui("No need to explain.")
        }
    }
    ...
    private func makeLocalPlaceholderReflection() -> String {
        switch mode {
        case .image:
            return AnkyLocalization.ui("I have the image. Your check-in has been saved.")
        case .write, .talk:
            let firstLine = text
                .split(separator: "\n")
                .map { $0.trimmingCharacters(in: .whitespacesAndNewlines) }
                .first { !$0.isEmpty } ?? AnkyLocalization.ui("what you shared")
            return AnkyLocalization.ui("I read this: %@. Your raw check-in has been saved.", firstLine)
        }
    }
}
...
private enum SpeechTranscriberError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .speechDenied:
            return AnkyLocalization.ui("Speech recognition permission is needed to talk with Anky.")
        case .microphoneDenied:
            return AnkyLocalization.ui("Microphone permission is needed to keep recording.")
        }
    }
}
