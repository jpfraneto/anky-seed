...
struct AnkyWitnessView: View {
    ...
    var body: some View {
        ZStack {
            if size == .companion {
                ...
                Circle()
                    .fill(
                        LinearGradient(
                            colors: [Color.ankyPaper.opacity(0.85), Color.ankyPaperDeep.opacity(0.70)],
        ...
        // The companion sprite breathes inside AnkySpriteView already — a
        // second, root-level repeatForever here lives one layer under the
        // presence overlay's .position, and its standing transaction
        // captures position changes at moments like the 8-minute reveal,
        // floating the companion up and down forever (feedback 2026-07-08).
        // Only the static glyph sizes keep the breath, scoped to the scale.
        .scaleEffect(size == .companion ? 1 : (breathing ? 1.035 : 0.985))
...
private struct AnkyDialoguePanel: View {
                    ...
                    ForEach(actions) { chatAction in
                        Button(action: chatAction.action) {
                            VStack(spacing: 2) {
                                ...
                                .foregroundStyle(chatAction.isPrimary ? Color.ankyInk : Color.ankyInkSoft)
                                .padding(.horizontal, isActionOnly ? 14 : 8)
                                .frame(maxWidth: .infinity)
                                .frame(height: isActionOnly ? 58 : (chatAction.subtitle == nil && chatAction.badge == nil ? 32 : 58))
                                .background(
                                    chatAction.isPrimary ? AnkyTheme.goldBright : Color.ankySlate.opacity(0.14),
                                    in: RoundedRectangle(cornerRadius: 4, style: .continuous)
                                )
                                .overlay(
                                    RoundedRectangle(cornerRadius: 4, style: .continuous)
                                        .stroke(chatAction.isPrimary ? Color.ankyInk.opacity(0.10) : AnkyTheme.gold.opacity(0.34), lineWidth: 1)
                                )
                        }
                        .buttonStyle(.plain)
                        .frame(maxWidth: .infinity)
    ...
    private var dialogueBackground: some View {
        RoundedRectangle(cornerRadius: 6, style: .continuous)
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 6, style: .continuous)
                    .stroke(isThinking ? AnkyTheme.gold.opacity(0.78) : Color.ankyInk.opacity(0.10), lineWidth: 1)
            )
            .overlay(
            ...
            .shadow(color: Color.ankyViolet.opacity(0.18), radius: 18, y: 10)
    }
}
    ...
    var body: some View {
        GeometryReader { geometry in
            Canvas { context, size in
                ...
                let blue = Color.ankySlate.opacity(0.28)
                let inset: CGFloat = 7
